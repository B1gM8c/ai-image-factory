    ...
    async fn pinned_codex_cli_preserves_exact_native_output_after_exit() {
        ...
        let bytes = crate::codex_app_server::run_codex_app_server(
            crate::codex_app_server::CodexAppServerRequest {
                ...
                prompt: "Invoke image_gen.imagegen exactly once now. Generate one minimal blue square icon on an opaque white background, with no text. Do not answer with text only; after the tool completes, stop.",
                input_paths: &[],
    ...
    async fn pinned_codex_cli_edit_preserves_exact_native_output_after_exit() {
        ...
        let bytes = crate::codex_app_server::run_codex_app_server(
            crate::codex_app_server::CodexAppServerRequest {
                ...
                workspace: &request_dir,
                codex_home: &request_codex_home,
                prompt: "Invoke image_gen.imagegen exactly once now. Edit the attached red square image into a blue circular icon on an opaque white background, preserving the centered composition. Do not answer with text only; after the tool completes, stop.",
